...
独立检索 Top-1/Top-5 是标准的逐查询检索指标。贪心、匈牙利、稳定匹配和 Sinkhorn 报告的是每条查询的单一**分配准确率**；分配准确率 Top-5 没有定义，因此不报告。全局算法会观察完整分数矩阵，而硬性一对一算法在标准 200×200 场景中利用了已知双射，所以这些列不能替代标准独立检索。
