...
本仓库研究能否将非侵入式 EEG 记录映射到视觉语义嵌入空间，并用它检索受试者观看过的图像。目前已验证的实验在 **THINGS-EEG2 Subject 08** 上，将可训练的脑编码器与经过 LoRA 适配的 CLIP 视觉编码器进行对齐，随后比较标准独立检索与可选的全局一对一匈牙利解码器。

> **范围说明。** 本 README 中经过验证的数值仅涵盖一名受试者（`sub-08`）和一个随机种子（`42`），并非十名受试者的平均结果。
...
- 报告固定的最终检查点结果，而不是选择测试集表现最好的 epoch。
- 同时提供确定性的逐查询评估，以及经过审计的匈牙利一对一分配消融实验。
- 提供单元测试、独立的检查点重新加载验证、逐查询预测结果和相似度矩阵来源记录。
...
正式结果使用第 25 个 epoch 后保存的模型，并在 200 个留出的查询和 200 张唯一图库图像上进行独立的保存/重新加载评估。

| 评估协议 | Top-1 / 分配准确率 | Top-5 |
|---|---:|---:|
| 标准逐查询独立检索 | **182/200 (91.0%)** | **199/200 (99.5%)** |
| 全局匈牙利一对一分配 | **200/200 (100.0%)** | 不适用 |

包含 200 张候选图像时，随机基线为 Top-1 0.5%、Top-5 2.5%。
...
在本次运行中，独立 Top-1 预测只覆盖了 183 张不同的图库图像。匈牙利解码改变了 18 个分配，将全部 18 个标准 Top-1 错误转换为正确匹配，同时没有把任何原本正确的匹配改错。预先声明的九种行/列排序产生了相同的映射分配，从而排除了依靠对齐顺序打破平局而得到 100% 结果的解释。

因此，推荐采用以下报告方式：

- **主要结果：**标准 Top-1 91.0%，标准 Top-5 99.5%。
- **次要消融结果：**全局一对一分配准确率 100.0%。
...
|---|---|
| 数据集 | THINGS-EEG2 |
| 已验证受试者 / 随机种子 | `sub-08` / `42` |
| 加载后的训练 EEG 张量 | `(16540, 4, 63, 250)` |
| 加载后的测试 EEG 张量 | `(200, 80, 63, 250)` |
...
│   ├── run_sub08_reproduction.sh   # 特定站点的复现封装脚本
│   ├── run_hungarian_evaluation.sh # 特定站点的匈牙利评估封装脚本
│   └── submit_*.slurm              # HKUST(GZ) SLURM 启动脚本
├── tests/
...
请从仓库根目录运行本节中的命令。正式实验使用 Linux、一张 NVIDIA A40，以及以下经过完整测试的软件栈：
...
以下命令无需依赖特定站点封装脚本中的路径，即可复现 Subject 08 配置：
...
如果使用新硬件或新准备的数据集，请在正式任务前先进行一步或一个 epoch 的冒烟测试。
...
评估器仍会写出标准逐查询指标，同时增加独立的受约束分配命名空间和 CSV：
...
- 标准评估会在独立重新加载模型后重复执行。
- 匈牙利评估会保存完整相似度矩阵、ID 顺序、哈希、转换记录和分配输出。
...
- 匈牙利解码需要完整的查询批次和已知的一对一图库先验，因此不是在线单查询检索协议。
